...
import { reuseSpecialProps, wrapInitializer, parseDependencyDeclaration, name, autoName, type, inject, autoInject, alsoInject, useInject, mergeInject, parseInjections, options, extra, initializer, constant, service, autoService, provider, autoProvider, handler, autoHandler, SPECIAL_PROPS } from './util';
...
describe('options', () => {
  it('should allow to decorate an initializer with options', () => {
    ...
    const newInitializer = inject(dependencies, options(baseOptions, aProvider, false));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
  ...
  it('should allow to decorate an initializer with options', () => {
    ...
    const newInitializer = inject(dependencies, options(baseOptions, options(rootOptions, aProvider), true));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], rootOptions);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.OPTIONS], { ...rootOptions,
      ...baseOptions
    });
  });
});
describe('name', () => {
  it('should allow to decorate an initializer with a name', () => {
    ...
    const baseName = 'hash';
    const newInitializer = inject(dependencies, options(baseOptions, name(baseName, aProvider)));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
...
describe('autoName', () => {
  it('should allow to decorate an initializer with its function name', () => {
    ...
    const baseName = 'hash';
    const newInitializer = inject(dependencies, options(baseOptions, autoName(async function hash() {})));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
  ...
  it('should allow to decorate an initializer with its init like function name', () => {
    ...
    const baseName = 'hash';
    const newInitializer = inject(dependencies, options(baseOptions, autoName(async function initHash() {})));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
  ...
  it('should allow to decorate an initializer with its initialize like function name', () => {
    ...
    const baseName = 'hash';
    const newInitializer = inject(dependencies, options(baseOptions, autoName(async function initializeHash() {})));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
  ...
  it('should allow to decorate a bounded initializer', () => {
    ...
    const baseName = 'hash';
    const newInitializer = autoName(inject(dependencies, options(baseOptions, async function initializeHash() {})));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
...
describe('extra', () => {
  ...
  it('should allow to decorate an initializer with additional extra infos', () => {
    ...
    const newInitializer = extra(baseExtraInformations, extra(additionalExtraInformations, aProvider), true);
    assert.notEqual(newInitializer, aProvider);
...
describe('type', () => {
  it('should allow to decorate an initializer with a type', () => {
    ...
    const newInitializer = inject(dependencies, options(baseOptions, name(baseName, type(baseType, aProvider))));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
...
describe('service', () => {
  it('should allow to create an initializer from a service builder', async () => {
    ...
    const baseName = 'hash';
    const baseType = 'service';
    const newInitializer = service(aServiceBuilder, baseName, dependencies, baseOptions);
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
  ...
  it('should allow to create an initializer from a service builder', async () => {
    ...
    const baseName = 'hash';
    const baseType = 'service';
    const newInitializer = service(name(baseName, inject(dependencies, options(baseOptions, aServiceBuilder))));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
...
describe('provider', () => {
  it('should allow to create an initializer from a provider builder', async () => {
    ...
    const newInitializer = provider(aServiceBuilder, baseName, dependencies, baseOptions);
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
  ...
  it('should allow to create an initializer from a provider builder', async () => {
    ...
    const baseName = 'hash';
    const baseType = 'provider';
    const newInitializer = provider(name(baseName, inject(dependencies, options(baseOptions, aServiceBuilder))));
    assert.notEqual(newInitializer, aProvider);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], dependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.OPTIONS], baseOptions);
...
//# sourceMappingURL=util.mocha.mjs.map