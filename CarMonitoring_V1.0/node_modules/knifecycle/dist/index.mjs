import { SPECIAL_PROPS, ALLOWED_INITIALIZER_TYPES, DECLARATION_SEPARATOR, OPTIONAL_FLAG, name, autoName, inject, useInject, mergeInject, autoInject, alsoInject, type, options, extra, reuseSpecialProps, initializer, constant, service, autoService, provider, autoProvider, handler, autoHandler, wrapInitializer, parseDependencyDeclaration, stringifyDependencyDeclaration } from './util';
...
class Knifecycle {
  ...
  register(initializer) {
    ...
    initializer[SPECIAL_PROPS.OPTIONS] = initializer[SPECIAL_PROPS.OPTIONS] || {};
    initializer[SPECIAL_PROPS.TYPE] = initializer[SPECIAL_PROPS.TYPE] || ALLOWED_INITIALIZER_TYPES[0];

    if (!initializer[SPECIAL_PROPS.NAME]) {
      throw new YError(E_ANONYMOUS_ANALYZER, initializer[SPECIAL_PROPS.NAME]);
    }

    if (initializer[SPECIAL_PROPS.NAME] === AUTOLOAD && !initializer[SPECIAL_PROPS.OPTIONS].singleton) {
    ...
    if (initializer[SPECIAL_PROPS.TYPE] === 'constant') {
      ...
      if (!initializer[SPECIAL_PROPS.OPTIONS].singleton) {
        throw new YError(E_NON_SINGLETON_CONSTANT_INITIALIZER, initializer[SPECIAL_PROPS.NAME]);
      }
...
export { SPECIAL_PROPS, DECLARATION_SEPARATOR, OPTIONAL_FLAG, Knifecycle, initializer, name, autoName, type, inject, useInject, mergeInject, autoInject, alsoInject, options, extra, reuseSpecialProps, wrapInitializer, constant, service, autoService, provider, autoProvider, handler, autoHandler, parseDependencyDeclaration, stringifyDependencyDeclaration };
