...
export async function initialize(services = {}) {${batches.map((batch, index) => `
  ...
  const batch${index} = {${batch.map(name => {
    ...
    ${name}: Promise.resolve(${name}),`;
      ...
      return `
    ${name}: ${dependenciesHash[name].__initializerName}({${dependenciesHash[name].__inject ? `${dependenciesHash[name].__inject.map(parseDependencyDeclaration).map(({
        serviceName,
        mappedName
      }) => `
...
`).join('')}
  return {${dependencies.map(parseDependencyDeclaration).map(({
      serviceName,
      mappedName
    }) => `
    ${serviceName}: services['${mappedName}'],`).join('')}
